  ...
  async getAssistantContext(apiKey: string) {
    ...
    const restaurant = await this.prisma.restaurant.findUniqueOrThrow({
      ...
      include: {
        ...
        bookingCutoffs: true,
        branches: { where: { isEnabled: true }, include: { bookingWindows: { where: { isEnabled: true }, orderBy: [{ weekday: "asc" }, { service: "asc" }] }, bookingExceptions: { where: { serviceDate: { gte: new Date(now.toISOString().slice(0, 10)) } }, orderBy: { serviceDate: "asc" } }, openingHours: { orderBy: [{ weekday: "asc" }, { startTime: "asc" }] }, rooms: { where: { isActive: true }, select: { name: true, description: true, isOutdoor: true, bookingPriority: true, bookingBlocks: { where: { serviceDate: { gte: new Date(now.toISOString().slice(0, 10)) } }, select: { serviceDate: true, turn: true }, orderBy: { serviceDate: "asc" } }, bookingRules: { where: { OR: [{ endsAt: null }, { endsAt: { gte: new Date(now.toISOString().slice(0, 10)) } }] }, select: { weekdays: true, turns: true, startsAt: true, endsAt: true } } }, orderBy: [{ bookingPriority: "asc" }, { createdAt: "asc" }] } }, orderBy: { createdAt: "asc" } }
      }
    });
