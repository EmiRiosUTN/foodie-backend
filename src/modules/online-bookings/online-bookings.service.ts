...
type Schedule = { isEnabled: boolean; startTime: string; endTime: string; intervalMin: number };
...
export class OnlineBookingsService {
  ...
  private async restaurantBySlug(slug: string) {
    const restaurant = await this.prisma.restaurant.findUnique({ where: { slug }, include: { onlineBooking: true, branches: { orderBy: { createdAt: "asc" } } } });
  ...
  private async scheduleFor(restaurantId: string, branchId: string, date: string, timezone: string): Promise<Schedule | null> {
    const exception = await this.prisma.onlineBookingException.findFirst({ where: { restaurantId, branchId, serviceDate: serviceDate(date) } });
    if (exception) {
      if (exception.isClosed || !exception.startTime || !exception.endTime || !exception.intervalMin) return null;
      return { isEnabled: true, startTime: exception.startTime!, endTime: exception.endTime!, intervalMin: exception.intervalMin! };
    }
    return this.prisma.onlineBookingSchedule.findFirst({ where: { restaurantId, branchId, weekday: weekday(date, timezone), isEnabled: true } });
  }
  ...
  private validateWindow(date: string, settings: { minAdvanceMinutes: number; maxAdvanceDays: number }, timezone: string) {
    const now = new Date();
    const max = new Date(now.getTime() + settings.maxAdvanceDays * 86_400_000);
  ...
  async saveOwnerConfig(user: RequestUser, input: { isEnabled: boolean; coverImageUrl?: string | null; whatsappPhone?: string | null; accentColor: string; minAdvanceMinutes: number; maxAdvanceDays: number; minPartySize: number; maxPartySize: number; branchDurations?: Array<{ branchId: string; durationMinutes: number }>; schedules: Array<{ branchId: string; weekday: number; isEnabled: boolean; startTime: string; endTime: string; intervalMin: number }>; exceptions: Array<{ branchId: string; serviceDate: string; isClosed: boolean; startTime?: string; endTime?: string; intervalMin?: number }> }) {
    ...
    await this.prisma.$transaction(async (tx) => {
      const settingsData = { isEnabled: input.isEnabled, coverImageUrl: input.coverImageUrl || null, whatsappPhone: input.whatsappPhone || null, accentColor: input.accentColor, minAdvanceMinutes: input.minAdvanceMinutes, maxAdvanceDays: input.maxAdvanceDays, minPartySize: input.minPartySize, maxPartySize: input.maxPartySize };
  ...
  async getPublicProfile(slug: string) {
    ...
    return { name: restaurant.name, slug: restaurant.slug, logoUrl: restaurant.profileImageUrl, coverImageUrl: restaurant.onlineBooking?.coverImageUrl || null, whatsappPhone: restaurant.onlineBooking?.whatsappPhone || null, accentColor: restaurant.onlineBooking?.accentColor || "#FF5A00", minPartySize: restaurant.onlineBooking?.minPartySize || 1, maxPartySize: restaurant.onlineBooking?.maxPartySize || 12, supportedFeatures, branches: restaurant.branches.map((branch) => ({ slug: branch.publicSlug, name: branch.name })) };
  }
  ...
  async availability(slug: string, input: { branch: string; date: string; partySize: number; preferredFeatures: PreferredFeature[] }, ip: string, skipLimit = false) {
    ...
    const schedule = await this.scheduleFor(restaurant.id, branch.id, input.date, branch.timezone);
    if (!schedule) return { date: input.date, partySize: input.partySize, slots: [] };
    ...
    for (let minute = timeToMinutes(schedule.startTime); minute < timeToMinutes(schedule.endTime); minute += schedule.intervalMin) {
      ...
      if (!this.meetsAdvance(input.date, time, settings.minAdvanceMinutes, branch.timezone)) continue;
  ...
  async createPublicReservation(slug: string, input: { branch: string; date: string; partySize: number; time: string; fullName: string; phone: string; notes?: string; preferredFeatures: PreferredFeature[]; website?: string }, ip: string) {
    ...
    this.validateWindow(input.date, settings, branch.timezone);
    const schedule = await this.scheduleFor(restaurant.id, branch.id, input.date, branch.timezone);
    if (!schedule || timeToMinutes(input.time) < timeToMinutes(schedule.startTime) || timeToMinutes(input.time) >= timeToMinutes(schedule.endTime) || (timeToMinutes(input.time) - timeToMinutes(schedule.startTime)) % schedule.intervalMin !== 0) throw new ConflictException({ code: "SLOT_UNAVAILABLE", message: "This time is no longer available" });
    if (!this.meetsAdvance(input.date, input.time, settings.minAdvanceMinutes, branch.timezone)) throw new ConflictException({ code: "SLOT_UNAVAILABLE", message: "This time is no longer available" });
